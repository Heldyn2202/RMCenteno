...
// Obtener secciones para el grado específico (sin filtros estrictos)
$sql = "
    SELECT DISTINCT 
        mp.id_seccion,
        s.nombre_seccion,
    ...
    WHERE (mp.estado = 'pendiente' OR mp.estado IS NULL OR mp.estado = '')
      AND g.grado = :grado
      AND g.nivel = :nivel
    GROUP BY mp.id_seccion, s.nombre_seccion, g.grado, g.nivel
...
if (empty($secciones)) {
    echo json_encode(['status' => 'warning', 'message' => 'No se encontraron secciones para este grado']);
    exit;
}
